...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">工程信息管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>工程信息列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">工程信息列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										工程名称
									</label>
									<input type="text" id="gongchengmingchengSearch" class="form-control form-control-sm" placeholder="请输入工程名称" aria-controls="tableId">
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('gongchengmingcheng')">工程名称<i id="gongchengmingchengIcon" class="fa fa-sort"></i></th>
											<th>图片</th>
											<th onclick="sort('gongchengmianji')">工程面积<i id="gongchengmianjiIcon" class="fa fa-sort"></i></th>
											<th>装修建材</th>
											<th>操作</th>
										</tr>
									</thead>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#gongchengmingchengSearch').val() != null && $('#gongchengmingchengSearch').val() != ''){
				searchForm.gongchengmingcheng ="%" +  $('#gongchengmingchengSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("gongchengxinxi/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				gongchengmingcheng : searchForm.gongchengmingcheng,	
			},(res)=>{
				if(res.code == 0){
					clear();
		...
		function setDataRow(item,number){
			...
			var gongchengmingchengCell  = document.createElement('td');

						 gongchengmingchengCell.innerHTML = item.gongchengmingcheng;

			row.appendChild(gongchengmingchengCell);
			...
			var gongchengmianjiCell  = document.createElement('td');

						 gongchengmianjiCell.innerHTML = item.gongchengmianji;

			row.appendChild(gongchengmianjiCell);
			var zhuangxiujiancaiCell  = document.createElement('td');

						  if(item.zhuangxiujiancai != null && item.zhuangxiujiancai != "" && item.zhuangxiujiancai.length >= 11){
			  	zhuangxiujiancaiCell.innerHTML = item.zhuangxiujiancai.substring(0, 10) + "..."; 
			  }else{
			  	zhuangxiujiancaiCell.innerHTML = item.zhuangxiujiancai;
			  }


			row.appendChild(zhuangxiujiancaiCell);
			var gongchengjieshaoCell  = document.createElement('td');
			...
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'gongchengdingdan','','','')");
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("gongchengxinxi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussgongchengxinxi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"gongchengxinxi");
